...
unify' (TTuple as) (TTuple bs) | length as == length bs = unifyList as bs
...
varBind v t | TVar v == t = return nullSubst
...
inferExpr e = case spanNode e of
    ...
    ELet x mty e1 e2 -> do
        ...
        env <- ask
        case mty of
            Just ty -> do s2 <- unify (apply s1 t1) ty
                          let t1' = apply s2 t1
    ...
    ERecord fields -> do
        ...
        let s = foldr compose nullSubst ss
            t = foldl (\r (l, t') -> MkSpan (initialPos "dummy") (initialPos "dummy") (TRecordExtend l (apply s t') r))
