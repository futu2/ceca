...
genIdentifier = do
    ...
    rest <- Gen.list (Range.linear 0 3) $
        Gen.choice
            [ Gen.element ['a' .. 'z']
            , Gen.element ['A' .. 'Z']
            , Gen.element ['0' .. '9']
...
genExtendRestrict = do
    ops <- Gen.list (Range.linear 1 3) $ Gen.choice
        [ do
            name <- genIdentifier
            return $ "-" <> name
        , do
...
prop_exprRoundtrip = property $ do
  ...
  where
    ...
    exprNodeEq (ERecord fs1) (ERecord fs2) =
      ...
      where fieldEq (n1, e1) (n2, e2) = n1 == n2 && exprNodeEq (spanNode e1) (spanNode e2)
    ...
    exprNodeEq (ELet n1 t1 e1 b1) (ELet n2 t2 e2 b2) =
      ...
      where typeOptEq Nothing Nothing = True
    ...
    patternEq (PRecord fs1 r1) (PRecord fs2 r2) =
        ...
        restPatEq r1 r2
      where fieldPatEq (n1, p1) (n2, p2) = n1 == n2 && patternEq (spanNode p1) (spanNode p2)
